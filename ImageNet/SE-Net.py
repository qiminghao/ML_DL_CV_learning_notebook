...
from keras.layers import Conv2D, Activation, MaxPooling2D, Dense, Dropout, Flatten, Concatenate, BatchNormalization, AveragePooling2D, Input, Add
...
def ResNet_50():
    ...
    x = Conv_Block(x, [512, 512, 2048], with_projection=False)
    # 1*1, 2048
    x = AveragePooling2D(pool_size=(7, 7), strides=(1, 1), padding='valid', data_format='channels_last')(x)
    x = Flatten()(x)
